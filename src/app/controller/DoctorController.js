...
class DoctorController {
    ...
    async store(req, res) {
        ...
        const { crm, nome, cep, telefone_fixo, telefone_celular, especialidades } = req.body
        ...
        const { logradouro, complemento, bairro, localidade, uf } = await (await axios.get(`http://viacep.com.br/ws/${cep}/json/`)).data;
        if(doctorExist && Doctor.deleted_at != null){
            return res.status(400).json({
                message: 'Esse usuário foi desativado'
            })
        }
        await Doctor.create({ crm, nome, cep, telefone_fixo, telefone_celular, logradouro, complemento, bairro, localidade, uf })
        ...
        for (let i = 0; i < especialidades.length; i++) {
            ...
            let find = await MedicalSpecialization.findByPk(especialidades[i])
      ...
      async update(req, res) {
        ...
        if (!crmDoctor)
          ...
          if (!(await schema.isValid(req.body))) {
            ...
            const { nome, cep, telefone_fixo, telefone_celular, especialidades } = req.body
            ...
            await crmDoctor.update({ nome, cep, telefone_fixo, telefone_celular, logradouro, complemento, bairro, localidade, uf })
